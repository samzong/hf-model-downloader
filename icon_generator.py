...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="从原始 PNG 文件生成 macOS (.icns) 和 Windows (.ico) 图标"
    )

    parser.add_argument(
        "--source", default="assets/icon.png", help="源图像路径 (默认: assets/icon.png)"
    )

    parser.add_argument(
        "--output-dir", default="assets", help="输出目录 (默认: assets)"
    )

    parser.add_argument(
        "--padding", type=float, default=15, help="内边距百分比 (默认: 15)"
    )

    parser.add_argument(
        "--radius", type=float, default=22, help="圆角半径百分比 (默认: 22)"
    )

    parser.add_argument(
        "--clear-cache", action="store_true", help="生成后清除 macOS 图标缓存"
    )
