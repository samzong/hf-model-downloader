...
class TestBasicE2E:
    ...
    def test_huggingface_tiny_model(self, temp_dir):
        ...
        assert os.path.exists(download_path), f"Download directory not created at {download_path}"
    ...
    def test_modelscope_model(self, temp_dir):
        ...
        if not success:
            ...
            if "authentication" in str(error_msg).lower() or "access" in str(error_msg).lower():
                pytest.skip(f"ModelScope model download requires authentication: {error_msg}")
            else:
    ...
    def test_modelscope_dataset(self, temp_dir):
        ...
        if not success:
            ...
            if any(keyword in str(error_msg).lower() for keyword in ["authentication", "access", "permission", "token"]):
                pytest.skip(f"ModelScope dataset download requires authentication: {error_msg}")
            else:
    ...
    def test_invalid_model_id(self, temp_dir):
        ...
        assert "not found" in error_msg.lower() or "does not exist" in error_msg.lower() or "404" in error_msg.lower()
